...
public class Inventory extends SubMenu {
	 ...
	 * 
	 * @param name     The submenu title that is to be displayed in the start menu.
	 * @param enabled  The description that is to be shown when the submenu is
	 *                 activated/enabled.
	 * @param disabled The description that is to be shown when the submenu is
	 *                 deactivated/disabled.
	 * @param game     The Game object that controls most of the actions/events the
	 *                 player has done when managing/using the Inventory.
	...
	/**
	 * Adds an item with its text description into the Inventory, being categorized
	 * into its relevant "pocket" of the player's bag.
	 * 
	 * @param itemText The item description of the item that is to be added into the
	 *                 Inventory.
	 * @param item     The item object that is to be added into the Inventory.
	 * @return Nothing.
	 */
	public void addItem(ItemText itemText) {
		...
		CHECK_LOOP: for (int i = 0; i < list.size(); i++) {
	 ...
	 * 
	 * @param keys The input keys the player is using.
	 * @return Itself.
	 */
	@Override
	 ...
	 * BufferStrategy}.
	 * 
	 * @param output   The display that is to be rendered.
	 * @param graphics The Graphics object that the main component creates using
	 *                 {@link java.awt.Canvas#getBufferStrategy() BufferStrategy}
	 *                 object.
	 * @return Nothing.
	 */
	@Override
	...
	/**
	 * Resets the position of the pointer and delete the selection menu commands of
	 * the item that the player is allowed to do.
	 * 
	 * @return Nothing.
	 */
	 ...
	 * 
	 * @return A list of all the items and their corresponding amount of the items
	 *         that the player is currently browsing in.
	 */
	...
	/**
	 * Obtains the list of items of the category the item belongs to in the
	 * Inventory.
	 * 
	 * @param item The target item that is used to get the list of items that the
	 *             targeted item belongs to.
	 * @return A list of all the items and their corresponding amount of the items
	 *         that the targeted item belongs to.
	 */
	...
	/**
	 * Retrieves the list of available commands the item allows from the given list
	 * of items.
	 * 
	 * @param list     The list of all the items that is currently being active and
	 *                 browsed by the player.
	 * 
	 * @param graphics The Graphics object that renders the custom font to the
	 *                 screen.
	 * 
	 * @return Nothing.
	 */
	private void renderItemMenuText(List<Map.Entry<Item, Integer>> list, Graphics graphics) {
		...
		case TOSS: {
			...
			if (item.getCategory() == null)
				break;
			if (item != null && list.get(itemCursor).getValue() != Integer.MAX_VALUE
					&& list.get(itemCursor).getValue() > 0 && this.selectionMenu.isEmpty()) {
			...
			try {
				...
				graphics.drawString(this.selectionMenu.get(0), (4 * Tileable.WIDTH + (3 * (Tileable.WIDTH / 2))),
						(Tileable.HEIGHT * 5 + 6));
				graphics.drawString(tossAmount, (4 * Tileable.WIDTH + ((11 - tossAmount.length()) * (Tileable.WIDTH / 2))),
						(Tileable.HEIGHT * 5 + 6));
			} catch (Exception e) {
			}
			...
			if (item != null && list.get(itemCursor).getValue() != Integer.MAX_VALUE) {
				...
				try {
					for (int i = 0; i < this.selectionMenu.size(); i++) {
						graphics.drawString(this.selectionMenu.get(i), (Tileable.WIDTH * 6 + 4),
	...
	/**
	 * Renders the background of the Inventory, which its rendering area is where
	 * the list of items are to be drawn on top of.
	 * 
	 * @param output The screen that the game used to draw for the player to see.
	 * @param x      The X coordinates of where the text is to be drawn at.
	 * @param y      The Y coordinates of where the text is to be drawn at.
	 * @param width  The width of the list box that is to be drawn.
	 * @param height The height of the lsit box that is too be drawn
	 * 
	 * @return Nothing.
	 */
	 ...
	 * 
	 * @param g The Graphics object that is passed to draw the text messages. It can
	 *          be a Graphics object that is created from a BufferedImage, or a
			...
			if (tick >= (byte) 0x4) {
				try {
					for (int i = 0; i < 5; i++) {
						...
						g.drawString(entry.getKey().getName(), 8 * (Tileable.WIDTH / 2),
						...
						if (value != Integer.MAX_VALUE && entry.getKey().getCategory() != null
							...
							g.drawString(string, 8 * (Tileable.WIDTH / 2) + ((12 - string.length()) * (Tileable.WIDTH / 2)),
